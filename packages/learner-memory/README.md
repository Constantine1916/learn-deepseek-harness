...
`append()` resolves only after the event line is durable. Stable EventId or CommandId values make retries idempotent. Reads reject malformed JSON, torn records, identity mismatches, duplicate identities, non-monotonic sequence numbers, and unsupported event versions instead of returning an empty history.

The package writes no DSH Session events and contributes no model-facing text. Model-request LearnerState snapshots are owned by the later teacher-prompt integration.
