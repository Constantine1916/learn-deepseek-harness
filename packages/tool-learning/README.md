...
`learning_request_hint` returns only the next persisted level for the active attempt. `learning_get_report` reads committed state and clearly separates browsing/started work, exercise completion, diagnostic waiver, and comprehensive integration validation.
